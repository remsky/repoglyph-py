---
type: "Directory"
title: "src/repoglyph"
description: "District `src/repoglyph/`: 11 files, 57.5 KB."
tags: ["repoglyph", "district"]
files: 11
bytes: 58931
---

# Stats

| metric | value |
| --- | --- |
| files | 11 (15% of the repo) |
| bytes | 57.5 KB |
| max depth | 2 |
| recent churn | 1,900 lines (24% of the window) |

# Largest files

| file | size | category |
| --- | --- | --- |
| `src/repoglyph/okf.py` | 17.7 KB | code |
| `src/repoglyph/cli.py` | 11.3 KB | code |
| `src/repoglyph/geometry.py` | 6.2 KB | code |
| `src/repoglyph/gitsource.py` | 6.1 KB | code |
| `src/repoglyph/models.py` | 5.5 KB | code |
| `src/repoglyph/palettes.py` | 4.7 KB | code |
| `src/repoglyph/palette.py` | 2.6 KB | code |
| `src/repoglyph/cache.py` | 2.4 KB | code |
| `src/repoglyph/__init__.py` | 614 B | code |
| `src/repoglyph/hashing.py` | 415 B | code |

# Recently active

| file | lines changed |
| --- | --- |
| `src/repoglyph/okf.py` | 550 |
| `src/repoglyph/cli.py` | 394 |
| `src/repoglyph/geometry.py` | 210 |
| `src/repoglyph/gitsource.py` | 178 |
| `src/repoglyph/models.py` | 164 |
| `src/repoglyph/palettes.py` | 130 |
| `src/repoglyph/cache.py` | 122 |
| `src/repoglyph/palette.py` | 107 |
| `src/repoglyph/__init__.py` | 23 |
| `src/repoglyph/hashing.py` | 14 |

# Files

Complete inventory: 11 files.

**`src/repoglyph/`**

- `__init__.py` (614 B)
- `__main__.py` (199 B)
- `cache.py` (2.4 KB)
- `cli.py` (11.3 KB)
- `geometry.py` (6.2 KB)
- `gitsource.py` (6.1 KB)
- `hashing.py` (415 B)
- `models.py` (5.5 KB)
- `okf.py` (17.7 KB)
- `palette.py` (2.6 KB)
- `palettes.py` (4.7 KB)

Context: [repository overview](../repository.md).
